...
if (typeof Blob !== "undefined") {
  (fileTypes as PropTypes.Validator<unknown>[]).push(
...
export const isLinkTarget = PropTypes.oneOf([
  "_self",
  "_blank",
  "_parent",
  "_top",
...
export const isPageIndex: Validator<number> = function isPageIndex(
  props,
  propName,
  componentName
) {
  ...
  if (isDefined(pageIndex)) {
    ...
    if (pageIndex + 1 > numPages) {
      return new Error(
        `Expected \`${propName}\` to be less or equal to ${numPages - 1}.`
      );
    }
...
export const isPageNumber: Validator<number> = function isPageNumber(
  ...
  if (isDefined(pageNumber)) {
    ...
    if (pageNumber > numPages) {
      return new Error(
        `Expected \`${propName}\` to be less or equal to ${numPages}.`
      );
    }
...
export const isRenderMode = PropTypes.oneOf([
  "canvas",
  "custom",
  "none",
  "svg",
